...
install:
	...
	modules='Getopt::Long File::Basename Parallel::ForkManager'; \
	for module in $$modules; do \
		perl -M$$module -e 1 2>/dev/null || cpanm --local-lib=~/perl5 $$module; \
	done
	@echo "Finished installing Perl modules to ~/perl5"
	@echo ""
	@echo "Installing samtools/bcftools/perl via conda..."
	@echo "Using $(CONDA) to create getbamdepth environment..."
	@$(CONDA) create -y -n getbamdepth -c bioconda samtools bcftools perl-parallel-forkmanager
	@echo ""
	@echo "Installation complete!"
	@echo "Activate conda environment: conda activate getbamdepth"
...
uninstall:
	...
	@echo "Finished uninstalling Perl modules..."
...
tests:
	...
	@$(CONDA) run -n getbamdepth ./getBamDepth --bed example/example-targets.bed --depth example/sample.depth > /tmp/test1.txt
	...
	@$(CONDA) run -n getbamdepth ./getBamDepth --bed example/example-targets.bed --bam example/sample.cram > /tmp/test2.txt
	...
	@$(CONDA) run -n getbamdepth ./getBamDepth --bed example/example-targets.bed --bam example/sample.bam --thresholds 5,10 > /tmp/test3.txt
